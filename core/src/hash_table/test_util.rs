...
use hash_table::{
    ...
    entry_meta::{
        tests::{
            test_attribute, test_attribute_b, test_entry_meta, test_entry_meta_for, test_value,
            test_value_b,
        },
        EntryMeta,
    },
...
pub fn test_modify_pair<HT: HashTable>(table: &mut HT) {
    ...
    assert_eq!(
        vec![
            EntryMeta::new(&test_keys(), &pair_1, LINK_NAME, &pair_2.key()),
            EntryMeta::new(
                &test_keys(),
                &pair_1,
                STATUS_NAME,
    ...
    assert_eq!(
        empty_vec,
        table
            .metas_for_entry(&pair_2)
            .expect("getting the metadata on a pair shouldn't fail")
    );
}

pub fn test_retract_entry<HT: HashTable>(table: &mut HT) {
    let pair = test_pair_unique();
    let empty_vec: Vec<EntryMeta> = Vec::new();
    ...
    assert_eq!(
        empty_vec,
        table
            .metas_for_entry(&pair)
            .expect("getting the metadata on a pair shouldn't fail")
    );

    table
        .retract_entry(&test_keys(), &pair)
...
pub fn test_entry_meta_round_trip<HT: HashTable>(table: &mut HT) {
    let meta = test_entry_meta();
...
fn test_metas_for_entry<HT: HashTable>(table: &mut HT) {
    let pair = test_pair_unique();
    let meta_a = test_entry_meta_for(&pair, &test_attribute(), &test_value());
    let meta_b = test_entry_meta_for(&pair, &test_attribute_b(), &test_value_b());
    let empty_vec: Vec<EntryMeta> = Vec::new();
...
pub fn standard_suite<HT: HashTable>(table: &mut HT) {
    ...
    test_retract_entry(table);

    test_entry_meta_round_trip(table);

    test_metas_for_entry(table);
